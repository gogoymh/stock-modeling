...
class Perceiver(nn.Module):
    def __init__(self, input_channels, input_shape, fourier_bands, device, latents=64, d_model=32, heads=8, latent_blocks=6, dropout=0.1, layers=8):
        super(Perceiver, self).__init__()
        self.fourier_features = fourier_features(
            shape=input_shape, bands=fourier_bands).to(device)
...
class PerceiverLogits(nn.Module):
    def __init__(self, input_channels, input_shape, output_features, device, fourier_bands=4, latents=64, d_model=32, heads=8, latent_blocks=6, dropout=0.1, layers=8):
...
if __name__ == "__main__":
    ...
    b = PerceiverLogits(3,(32,32),10, device, latents=128).to(device)
