...
df = marcap_data('2015-06-15', '2021-06-09')
...
for i in tqdm(range(len_stocks)):
    ...
    for j in range(1, (len(df_tmp)-3)):
        ...
        if df_tmp.iloc[(j+1),2] == 0:
            ...
            purchase = min(df_tmp.iloc[(j+1),2], (df_tmp.iloc[(j+1),2]+df_tmp.iloc[(j+1),1])/2)
            
            profit = ((df_tmp.iloc[(j+2),3] - purchase)/purchase)*100
            stop = ((df_tmp.iloc[(j+2),1] - purchase)/purchase)*100
        ...
        name = os.path.join("/home/DATA/ymh/s_modeling/data/cumulative7","input_%08d_%d_%f_%f_" % (cnt, buy, profit, stop))
