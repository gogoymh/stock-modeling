...
for i in tqdm(range(len_stocks)):
    ...
    for j in range(start, (len(df)-3)):
        ...
        if df.iloc[(j+1),0] == 0:
            ...
            stop_1 = ((df.iloc[(j+1),2] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100
            stop_2 = ((df.iloc[(j+2),2] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100
            stop_3 = ((df.iloc[(j+3),2] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100
            
            stop = min(stop_1, stop_2, stop_3)
        ...
        name = os.path.join("/home/DATA/ymh/s_modeling/data/input_v6","input_%08d_%d_%f_%f_" % (cnt, buy, profit, stop))
