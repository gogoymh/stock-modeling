...
class HB_transformer(nn.Module):
    def __init__(self, shape, device, output=1, d_models=16, dropout=0.1, heads=4, fourier_band=4, latent1_len=64, repeat=1, share=1):
        ...
        ## Layers ------------------------------------------------------------------------- ##
        self.attention_init = Operation(d_models, heads, repeat, dropout)
        self.attention_several = nn.ModuleList([Operation(d_models, heads, repeat, dropout) for _ in range(self.share)])
