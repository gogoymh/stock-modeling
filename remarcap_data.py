...
for i in tqdm(range(len_stocks)):
    ...
    for j in range(1, (len(df_tmp)-3)):
        oldday_path = os.path.join(save_path, str(df_tmp.index.values[j-1])[:10])
        ...
        if df_tmp.iloc[(j+1),2] == 0:
            buy = 0
            profit = 0
            stop = 0
        else:
            ## 오늘 시가 대비 앞으로 3일내 고가 5% 이상 상승
            profit_1 = ((df_tmp.iloc[(j+1),3] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 오늘 고가 오늘 시가 대비
            profit_2 = ((df_tmp.iloc[(j+2),3] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 내일 고가 오늘 시가 대비
            profit_3 = ((df_tmp.iloc[(j+3),3] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 모레 고가 오늘 시가 대비
            
            stop_1 = ((df_tmp.iloc[(j+1),1] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 오늘 종가 오늘 시가 대비
            stop_2 = ((df_tmp.iloc[(j+2),1] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 내일 종가 오늘 시가 대비
            stop_3 = ((df_tmp.iloc[(j+3),1] - df_tmp.iloc[(j+1),2])/df_tmp.iloc[(j+1),2])*100 # 모레 종가 오늘 시가 대비
        ...
        name = os.path.join("/home/DATA/ymh/s_modeling/data/cumulative4","input_%08d_%d_%f_%f_" % (cnt, buy, profit, stop))
