...
class AttentionBlock(nn.Module):
    ...
    def forward(self, q_input, kv):
        ...
        q_mid = self.dropout(q) #+ q_input
        
        q = self.layer_norm_q2(q_mid)
        q = self.gelu(q)
        q = self.linear2(q)
        q = self.dropout(q) + q_input# + q_mid
...
class HB_transformer(nn.Module):
    def __init__(self, shape, device, d_models=16, dropout=0.1, heads=4, fourier_band=4, latent1_len=64, repeat=1, share=1):
        ...
        ## Learnable Latent ------------------------------------------------------------------------- ##
        self.latent1 = nn.Parameter(torch.randn(latent1_len, d_models)) # (64, 5+pec)
        self.latent2 = nn.Parameter(torch.randn(input_len, channel)) # (3062, 5)
    ...
    def forward(self, x): # (B, 3062, 5)
        ...
        latent2 = self.latent2.unsqueeze(0).expand((B,) + self.latent2.shape) # (B, 3062, 5)
        latent2 = latent2.permute(1,0,2)  # (3062, B, 5)
        
        latent2 = torch.cat((latent2, pos), dim=-1) # (3062, B, 5+pec)
        latent2 = self.embed2(latent2) # (3062, B, d_models)
        ...
        out = self.cross2(latent2, bottleneck) # (3062, B, d_models)
        
        for _ in range(self.share):
            bottleneck = self.cross3(bottleneck, x) # (64, B, d_models)
