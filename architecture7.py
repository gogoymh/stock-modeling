...
class HB_transformer(nn.Module):
    def __init__(self, shape, device, output=1, d_models=32, depth=18, mlp_dim=16, dropout=0.):
        ...
        ## Positional Encoding ------------------------------------------------------------------------- ##
        self.encoding = fourier_encoding((input_len,), bands=4).to(device)
    ...
    def forward(self, x): # (B, 7, 5)
        B = x.shape[0]
        
        pos = self.encoding.unsqueeze(0).expand((B,) + self.encoding.shape) # (B, pec, 7)
        pos = pos.permute(0,2,1) # (B, 7, pec)
        
        x = torch.cat((x, pos), dim=-1) # (B, 7, 5+pec)
        x = self.embed1(x) # (B, 7, d_models)
        
        x = self.model(x) # (B, 7, d_models)
...
if __name__ == "__main__":
    ...
    a = torch.randn((8,7,5)).to(device)
    b = HB_transformer((5,7)).to(device)
