...
class HB_transformer(nn.Module):
    def __init__(self, shape, device, output=1, d_models=16, dropout=0.1, heads=4, fourier_band=4, latent1_len=64, repeat=1, share=1):
        ...
        self.latent2 = nn.Parameter(torch.randn(input_len, channel))
        self.embed2 = nn.Linear(channel+pec, d_models)
        
        ## Layers ------------------------------------------------------------------------- ##
        self.encoder = nn.ModuleList([Encoder(d_models, heads, repeat, dropout) for _ in range(self.share)])
        self.decoder = nn.ModuleList([CrossTransformer(d_models, 1, dropout) for _ in range(self.share)])
    ...
    def forward(self, x): # (B, 3062, 5)
        ...
        latent2 = self.latent2.unsqueeze(0).expand((B,) + self.latent2.shape).permute(1,0,2) # (3062, B, 5)
        latent2 = torch.cat((latent2, pos), dim=-1)
        latent2 = self.embed2(latent2)
        
        for encoding in self.encoder:
            latent1 = encoding(latent1, x)
        
        for decoding in self.decoder:
            latent2 = decoding(latent2, latent1)
        
        out = self.norm(latent2)
        out = self.gelu(out)
...
if __name__ == "__main__":
    ...
    b = HB_transformer((5,3062), device, repeat=4, share=6, d_models=32).to(device)
