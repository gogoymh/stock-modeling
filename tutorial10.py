...
for i in tqdm(range(len_stocks)):
    ...
    for j in range(start, (len(df)-5)):
        ...
        if df.iloc[(j+1),0] == 0:
            ...
            stop_1 = ((df.iloc[(j+1),3] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100 # 오늘 종가 오늘 시가 대비
            stop_2 = ((df.iloc[(j+2),3] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100 # 내일 종가 오늘 시가 대비
            stop_3 = ((df.iloc[(j+3),3] - df.iloc[(j+1),0])/df.iloc[(j+1),0])*100 # 모레 종가 오늘 시가 대비
            ...
            if profit_1 >= 5:
                if profit_2 >= 5:
                    if profit_3 >= 5:
                        ...
                        buy = 1
                        profit = max(profit_1, profit_2)
                        stop = stop_2
                else:
                    if profit_3 >= 5:
                        buy = 2
                        profit = max(profit_1, profit_3)
                        stop = stop_3
                    else:
                        buy = 3
                        profit = profit_1
        ...
        name = os.path.join("/home/DATA/ymh/s_modeling/data/input_v14","input_%08d_%d_%f_%f_" % (cnt, buy, profit, stop))
