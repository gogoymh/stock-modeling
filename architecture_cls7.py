...
class HB_transformer(nn.Module):
    def __init__(self, shape, device, output=1, d_models=32, depth=36, mlp_dim=16, dropout=0, fourier_band=4):
        ...
        ## Positional Encoding ------------------------------------------------------------------------- ##
        self.positional_encoding = nn.Parameter(torch.randn(input_len, channel))        
        self.embed1 = nn.Linear(channel, d_models)
    ...
    def forward(self, x): # (B, 3062, 5)
        ...
        pos = self.positional_encoding.unsqueeze(0).expand((B,) + self.positional_encoding.shape) # (B, 3062, 5)
